...
type Frequency   = "monthly" | "quarterly" | "biannual" | "yearly" | "custom";
type ValidToMode = "none" | "month" | "count" | "yearend";
...
interface RecurringFormProps {
  initialValues?:    RecurringDoc | null;
  validFrom?:        string;
...
function emptyForm(validFrom?: string): RecurringFormState {
  return {
    description:     "",
    subcategoryId:   "",
    subcategoryName: "",
...
export function RecurringForm({ initialValues, validFrom, activeBudgetMonth, onSubmit, onCancel, isSaving, mode = "add" }: RecurringFormProps) {
  ...
  function resolveInitialCurrency(stored?: string) {
  ...
  const [form, setForm] = useState<RecurringFormState>(() => {
    if (initialValues) {
      // Edit mode — get active cost for current month. The server doc is
      // loosely typed (index signature), so read its fields via `iv`.
      ...
      return {
        ...
        subcategoryId:   iv.subcategoryId || "",
        subcategoryName: iv.subcategoryName || "",
        ...
        tags:            iv.tags || [],
        priority:        iv.priority || 2,
        amount:          String(activeCost?.amount ?? ""),
        currency:        cur.currency,
        customCurrency:  cur.customCurrency,
        fxRate:          activeCost?.fxRate || 1,
        validToMode:     iv.validTo ? "month" : "none",
        validToDate:     iv.validTo
          ? (() => { const [y, m] = iv.validTo.split("-").map(Number); return new Date(y, m - 1, 1); })()
          : null,
        monthsCount:     "",
        validFrom:       activeBudgetMonth || iv.costs?.[0]?.validFrom || "",
      };
    }
  ...
  const handleRateReady = useCallback(({ activeRate, resolvedCurrency }: { activeRate: number | null; resolvedCurrency: string }) => {
    const rate = activeRate ?? 1;
    const key = `${resolvedCurrency}_${rate}`;
    if (lastRateRef.current === key) return;
    lastRateRef.current = key;
    setRateInfo({ activeRate: rate, resolvedCurrency });
    setForm(f => ({ ...f, fxRate: rate }));
  }, []);

  function set(k: keyof RecurringFormState, v: any) { setForm(p => ({ ...p, [k]: v })); }

  function toggleActiveMonth(m: number) {
    setForm(p => ({
      ...p,
...
function handleSubmit() {
  ...
  if (form.plannedDay < 1 || form.plannedDay > 31) {
    ...
    <div>
      ...
      <div style={frow}>
        ...
        <SubcategorySelect
          ...
          filter={(sub, cat) => cat.type === "SAVING" ? true : sub.canBeRecurring === true}
