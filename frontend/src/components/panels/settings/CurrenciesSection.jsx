      ...
      <CollapsibleSection title="💱 Waluty" defaultOpen={false}>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: 16 }}>
          {activeCurrencies.map(c => (
            <div key={c.code} style={chip(true)}>
              <EditableLabel
                value={c.code}
                onSave={newCode => updateCurrency(c.code, { code: newCode })}
                fontSize={13}
                fontWeight={700}
              />
              <EditableLabel
                value={c.name}
                onSave={newName => updateCurrency(c.code, { name: newName })}
                fontSize={12}
                fontWeight={400}
              />
