...
// ── Styles ────────────────────────────────────────────────────
const s = {
  panel:      { padding: "0 0 40px 0", maxWidth: 1280, margin: "0 auto", width: "100%" } as React.CSSProperties,
  ...
  formRow:    { marginBottom: 14 } as React.CSSProperties,
  modal:      { position: "fixed", inset: 0, zIndex: 1000, background: "rgba(0,0,0,0.75)", display: "flex", alignItems: "center", justifyContent: "center" } as React.CSSProperties,
  modalBox:   { background: c.surface, border: `1px solid ${c.border}`, borderRadius: 14, padding: "24px 28px", maxWidth: 480, width: "90vw", maxHeight: "85vh", overflowY: "auto" } as React.CSSProperties,
};
...
function emptyForm(): VoucherFormState {
  return {
...
function VoucherForm({ initial, onSubmit, onCancel, isSaving, mode = "add" }: VoucherFormProps) {
  const [form, setForm] = useState<VoucherFormState>((initial as VoucherFormState | undefined) ?? emptyForm());
  ...
  function handleSubmit() {
    ...
    if (isPercent) {
      const pct = parseInt(String(form.percentValue), 10);
      ...
      const val = parseFloat(String(form.initialValue));
              ...
              <>
                ...
                <div style={{ fontSize: 11, color: c.textMuted, marginTop: 4 }}>
                  ...
                  {" · "}Użyto: <strong style={{ color: c.orange }}>
                    {Math.max(0, Number(form.initialValue) - (typeof form.remainingValue === "number" ? form.remainingValue : Number(form.initialValue)))} PLN
                  </strong>
                </div>
              </>
            )
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }} data-vform-cols>
        ...
        <div style={s.formRow}>
          ...
          {(() => {
            ...
            const inMonths = (n: number) => new Date(base.getFullYear(), base.getMonth() + n, base.getDate());
...
// ── UsageHistory ─────────────────────────────────────────────
interface VoucherUsage { amount: number; usedAt?: string; description?: string }
function UsageHistory({ entries }: { entries: VoucherUsage[] }) {
...
interface VoucherCardProps {
  ...
  onArchive:  (v: Voucher) => void;
  warnDays?:  number;
}
function VoucherCard({ v, onEdit, onArchive, warnDays = 14 }: VoucherCardProps) {
...
export default function PanelVouchers() {
  ...
  const [archiveTarget, setArchiveTarget] = useState<Voucher | null>(null);
