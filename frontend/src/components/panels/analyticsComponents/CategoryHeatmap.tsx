...
export function CategoryHeatmap({ rows, months, onClick }: CategoryHeatmapProps) {
  ...
  return (
    <div style={{ background: c.surface, border: `1px solid ${c.border}`, borderRadius: 12, padding: "12px", overflowX: "auto" }}>
      <table style={{ borderCollapse: "separate", borderSpacing: 2, minWidth: "100%" }}>
        <thead>
          <tr>
            <th style={{ minWidth: 160, padding: "6px 10px", textAlign: "left", fontSize: 11, color: c.textMuted, textTransform: "uppercase", letterSpacing: "0.6px", fontWeight: 700 }}>
              Kategoria
            </th>
            {months.map(m => (
              <th key={m} style={{ minWidth: 70, padding: "6px 4px", textAlign: "center", fontSize: 10, color: c.textMuted, fontWeight: 700 }}>
                {m.slice(2)}
              </th>
            ))}
          </tr>
          ...
          {rows.map(row => {
            ...
            return (
              <tr key={row.categoryId}>
                <td style={{ padding: "6px 10px", fontSize: 12, color: c.textBody, fontWeight: 600, whiteSpace: "nowrap" }}>
                  {row.icon} {row.categoryName}
                  <span style={{ marginLeft: 6, color: c.textMuted, fontSize: 10, fontWeight: 400 }}>
                    {fmt(rowTotal)}
                  </span>
                </td>
                {months.map(m => {
                  ...
                  return (
                    <td
                      ...
                      style={{
                        ...
                        borderRadius: 6,
                        fontSize: 10,
                        color: value > maxValue * 0.5 ? c.white : c.textTertiary,
      ...
      <div style={{ marginTop: 10, display: "flex", gap: 8, alignItems: "center", fontSize: 10, color: c.textMuted }}>
        Skala:
