...
interface PieChartProps {
  ...
  labelResolver?: (cat: string) => string;
}
export function PieChart({ data, total, onSliceClick, labelResolver }: PieChartProps) {
  ...
  const resolve = (cat: string) => labelResolver ? labelResolver(cat) : cat;
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 4, justifyContent: "center", minWidth: 0 }}>
        {slices.map((sl, i) => {
          ...
          return (
            <div key={i}
              ...
              <span style={{ color: isActive ? c.text : c.textTertiary, fontSize: 11, flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                {resolve(sl.cat)}
              </span>
              <span style={{ color: isActive ? sl.color : c.textSecondary, fontWeight: isActive ? 700 : 500, fontSize: 11, whiteSpace: "nowrap" }}>
        ...
        {!active && (
          <div style={{ marginTop: 6, paddingTop: 6, borderTop: `1px solid ${c.border}`, display: "flex", justifyContent: "space-between" }}>
            <span style={{ color: c.textMuted, fontSize: 11 }}>Razem</span>
            <span style={{ color: c.success, fontWeight: 700, fontSize: 11 }}>{fmt(total)}</span>
          </div>
        )}
      </div>
    </div>
...
export function Gauge({ value, max, label, color }: GaugeProps) {
  ...
  const barColor = over ? c.danger : (color || c.success);
  return (
    <div style={{ marginBottom: 10 }}>
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
        <span style={{ color: c.textTertiary, fontSize: 12 }}>{label}</span>
        <span style={{ color: over ? c.danger : c.text, fontSize: 12, fontWeight: 700 }}>
          {fmt(value)} / {fmt(max)}
        </span>
      </div>
...
interface BarChartProps {
  data:   Array<{ value: number; label: ReactNode }>;
  color?: string;
}
export function BarChart({ data, color = c.success }: BarChartProps) {
  const max = Math.max(...data.map(d => d.value), 1);
  return (
    <div style={{ display: "flex", alignItems: "flex-end", gap: 4, height: 60 }}>
      {data.map((d, i) => (
        <div key={i} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 2 }}>
          <div style={{ width: "100%", background: c.border, borderRadius: 3, height: 46, display: "flex", alignItems: "flex-end" }}>
            <div style={{ width: "100%", height: `${(d.value / max) * 46}px`, background: color, borderRadius: 3, minHeight: d.value > 0 ? 2 : 0 }} />
          </div>
          <div style={{ fontSize: 8, color: c.textMuted }}>{d.label}</div>
        </div>
      ))}
    </div>
  );
...
export function Badge({ children, color = c.success }: BadgeProps) {
  return (
...
export function TagChip({ children }: { children: ReactNode }) {
  return (
    <span style={{ background: alpha(c.voucher, "22"), color: c.voucher, borderRadius: 5, padding: "1px 6px", fontSize: 10, fontWeight: 600 }}>
      {children}
    </span>
  );
}
...
export function Toggle({ value, onChange, label, color = c.success }: ToggleProps) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 8, cursor: "pointer", color: value ? color : c.textMuted, fontSize: 13, fontWeight: 600 }}
      onClick={() => onChange(!value)}>
      <div style={{ width: 36, height: 20, background: value ? color : c.border, border: `2px solid ${value ? color : c.borderStrong}`, borderRadius: 99, position: "relative", transition: "all 0.2s", flexShrink: 0 }}>
        <div style={{ position: "absolute", top: 2, left: value ? 16 : 2, width: 12, height: 12, background: c.white, borderRadius: "50%", transition: "left 0.2s" }} />
      </div>
      {label}
    </div>
  );
...
export function CollapsibleSection({ title, children, defaultOpen = true, style: extraStyle = {} }: CollapsibleSectionProps) {
  const [open, setOpen] = useState(defaultOpen);
  const cardStyle: CSSProperties = { background: c.surface, border: `1px solid ${c.border}`, borderRadius: 16, padding: 16, marginBottom: 12 };
  return (
    <div style={{ ...cardStyle, marginTop: 4, ...extraStyle }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", cursor: "pointer", marginBottom: open ? 14 : 0 }}
        onClick={() => setOpen(v => !v)}>
        <span style={{ fontWeight: 700, color: c.textTertiary, fontSize: 12, textTransform: "uppercase", letterSpacing: "0.5px" }}>{title}</span>
